...
import com.github.alesaudate.samples.reactive.carapp.interfaces.incoming.PassengerAPI
...
class TravelRequestMapper(
    ...
    fun map(travelRequest: TravelRequest) : Mono<TravelRequestOutput> {
        return Mono.just(TravelRequestOutput(
            id = travelRequest.id!!,
    ...
    fun buildOutputModel(travelRequest: TravelRequest, output: TravelRequestOutput): Mono<EntityModel<TravelRequestOutput>> {
        ...
        val link = Link.of("/passengers/${travelRequest.passenger.id.toString()}", "passenger")
